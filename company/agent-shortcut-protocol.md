...
- 此时应使用全局 `~/.claude/CLAUDE.md` 与 `E:/project/AIProjet/ai-memory`
...
强制前置步骤（每次执行 `CC_NEXT` 必须先做）：
1. 检测项目目录是否存在 `CLAUDE.md`、`AGENTS.md`、`tasks/`、`workflows/`
2. 如果以上任意一个不存在 → 判定为新项目，必须先读取 `company/project-bootstrap-standard.md`，然后按该文件执行骨架生成
3. 如果以上全部存在 → 判定为老项目，走"老项目流程"
4. 禁止：骨架未落完时直接输出任务卡
...
强制前置步骤：必须先读取 `company/pricing-principles.md`
...
强制前置步骤：必须先读取 `company/quality-bar.md`
...
- 判断哪些经验留在项目内
- 判断哪些经验升级到 `E:/project/AIProjet/ai-memory`
